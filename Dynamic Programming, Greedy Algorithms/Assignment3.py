...
def compute_makespan(times, m, assign):
    # Create an array to hold the total time for each processor
    processor_times = [0] * m

    # Sum up the times for each job assigned to each processor
    for i in range(len(times)):
        processor = assign[i] - 1  # Adjusting index since processors are 1-indexed in the assignment
        processor_times[processor] += times[i]

    # The makespan is the maximum time taken by any processor
...
def greedy_makespan_min(times, m):
    n = len(times)
    # Initialize the assignments and the current load of each processor
    assignments = [None] * n
    processor_loads = [(0, i) for i in range(m)]  # (load, processor_id) tuples for min-heap
    heapq.heapify(processor_loads)  # Convert to min-heap

    for i in range(n):
        # Pop the processor with the least load
        load, processor_id = heapq.heappop(processor_loads)
        # Assign the job to this processor
        assignments[i] = processor_id
        # Update the load of this processor
        updated_load = (load + times[i], processor_id)
        heapq.heappush(processor_loads, updated_load)

    # The makespan is the maximum load among all processors
    makespan = max(processor_loads)[0]
    return assignments, makespan

# Example use of the function
times_example = [3, 2, 2, 5, 4, 1, 3]  # Example job times
m_example = 3  # Example number of processors

# Compute the assignment and makespan using the greedy algorithm
assignment_result, makespan_result = greedy_makespan_min(times_example, m_example)
assignment_result, makespan_result
