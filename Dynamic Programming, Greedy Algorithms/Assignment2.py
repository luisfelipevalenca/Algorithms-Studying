...
def lssLength(a, i, j):
    ...
    if aj is not None and abs(a[i] - aj) > 1:
        # Cannot choose a[i], skip it
        return lssLength(a, i + 1, j)
    else:
        # Two options: take a[i] or skip a[i]
        take = 1 + lssLength(a, i + 1, i) if aj is None or abs(a[i] - aj) <= 1 else 0
...
def memoizeLSS(a):
    ...
    for i in range(n - 1, -1, -1):  # Iterate backwards from n-1 to 0
        for j in range(i - 1, -2, -1):  # Iterate j from i-1 to -1
            ...
            if aj is not None and abs(a[i] - aj) > 1:
                ...
                # Two options: take a[i] or skip a[i]
                take = 1 + T[(i + 1, i)] if aj is None or abs(a[i] - aj) <= 1 else 0
                skip = T[(i + 1, j)]
                T[(i, j)] = max(take, skip)
        ...
        for i in range(n-1, -1, -1):
            for j in range(i-1, -2, -1):
                ...
                if aj is not None and abs(a[i] - aj) > 1:
                    ...
                    take_len, take_seq = T[(i+1, i)]
                    skip_len, skip_seq = T[(i+1, j)]

                    if aj is None or abs(a[i] - aj) <= 1:
                        take_len += 1
                        take_seq = [a[i]] + take_seq

                    if take_len > skip_len:
                        T[(i, j)] = (take_len, take_seq)
                    else:
                        T[(i, j)] = (skip_len, skip_seq)
...
a = [1, 4, 2, -2, 0, -1, 2, 3]
lss = computeLSS(a)
print("Longest Stable Subsequence:", lss)
...
def targetSum(S, i, tgt):
    if tgt < 0:
        return float('inf')  # If target becomes negative, it's not a valid solution
    if i > len(S) - 1:
        return tgt  # If we've considered all elements, return the remaining target
    
    # Recurrence relation: Choose minimum of two options - take S[i] or skip S[i]
    take = targetSum(S, i + 1, tgt - S[i])  # Take the current element
    skip = targetSum(S, i + 1, tgt)  # Skip the current element
...
def memoTargetSum(S, tgt):
    ...
    # Function to safely lookup values in memo table
    ...
    for i in range(k - 1, -1, -1):
        for j in range(tgt + 1):
            take = lookupMemoTable(i + 1, j - S[i])  # Take the current element
            skip = lookupMemoTable(i + 1, j)  # Skip the current element
            T[(i, j)] = min(take, skip)
...
S = [1, 2, 3, 4, 5, 10]
N = 20
memo_table = memoTargetSum(S, N)
print("Memoized Table:", memo_table)
print("Minimum difference:", memo_table[(0, N)])
...
def getBestTargetSum(S, tgt):
    ...
    # Initialize memo table
    T = {}
    choice = {}  # To record choices made

    # Base case
    for j in range(tgt + 1):
        T[(k, j)] = j

    # Fill memo table and record choices
    for i in range(k - 1, -1, -1):
        for j in range(tgt + 1):
            take = T[(i + 1, j - S[i])] if j - S[i] >= 0 else float('inf')
            skip = T[(i + 1, j)]

            if take < skip:  # If taking the current element is better
                T[(i, j)] = take
                choice[(i, j)] = True
            else:
                T[(i, j)] = skip
    ...
    remaining_tgt = tgt
    for i in range(k):
        if choice.get((i, remaining_tgt), False):
            res.append(S[i])
            remaining_tgt -= S[i]
...
S = [1, 2, 3, 4, 5, 10]
N = 20
best_subset = getBestTargetSum(S, N)
