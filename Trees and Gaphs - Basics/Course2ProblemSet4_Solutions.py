...
def computeShortestPath(graph, source_coordinates, dest_coordinates):
    ...
    while not q.is_empty():
        ...
        if u.x == d.x and u.y == d.y:
            ...
            break
        for neighbor, weight in graph.get_list_of_neighbors(u):
            if not neighbor.processed and neighbor.d > u.d + weight:
                neighbor.d = u.d + weight
                neighbor.pi = u
                if neighbor.idx_in_priority_queue == -1:
                    q.insert(neighbor)
                else:
                    q.update_vertex_weight(neighbor)
