...
class UndirectedGraph:
    ...
    def dfs_visit(self, i, dfs_timer, discovery_times, finish_times,
        ...
        # Sort the neighbors in increasing order for consistent results.
        neighbors = sorted(list(self.get_neighboring_vertices(i)))
        
        for neighbor in neighbors:
            if discovery_times[neighbor] is None:
                # i to neighbor is a tree edge
...
def num_connected_components(g): # g is an UndirectedGraph class
    def dfs_visit(i, dfs_timer, discovery_times, finish_times, dfs_tree_parent, dfs_back_edges):
        assert 0 <= i < g.n  # Use g.n instead of self.n
        assert discovery_times[i] is None
        assert finish_times[i] is None
        discovery_times[i] = dfs_timer.get()
        dfs_timer.increment()

        neighbors = sorted(list(g.get_neighboring_vertices(i)))  # Use g.get_neighboring_vertices

        for neighbor in neighbors:
            if discovery_times[neighbor] is None:
                dfs_tree_parent[neighbor] = i
                dfs_visit(neighbor, dfs_timer, discovery_times, finish_times, dfs_tree_parent, dfs_back_edges)
            elif finish_times[neighbor] is None:
                dfs_back_edges.append((i, neighbor))
    ...
    dfs_timer = DFSTimeCounter()
    discovery_times = [None] * g.n  # Use g.n instead of self.n
    finish_times = [None] * g.n  # Use g.n instead of self.n
    dfs_tree_parents = [None] * g.n  # Use g.n instead of self.n
    dfs_back_edges = []
    ...
    for i in range(g.n):
        if discovery_times[i] is None:
            dfs_visit(i, dfs_timer, discovery_times, finish_times, dfs_tree_parents, dfs_back_edges)
            num_components += 1
...
def find_all_nodes_in_cycle(g):
    ...
    def dfs_visit(i, dfs_timer, discovery_times, finish_times, dfs_tree_parent, dfs_back_edges):
        nonlocal set_of_nodes  # To access the set_of_nodes from the outer function
        ...
        for neighbor in neighbors:
            if discovery_times[neighbor] is None:
                dfs_tree_parent[neighbor] = i
                dfs_visit(neighbor, dfs_timer, discovery_times, finish_times, dfs_tree_parent, dfs_back_edges)
            elif finish_times[neighbor] is None and dfs_tree_parent[i] != neighbor:
                # Found a non-trivial back edge, i.e., a cycle
                # Add nodes in the cycle to the set_of_nodes
                current_node = i
                while current_node != neighbor:
                    set_of_nodes.add(current_node)
                    current_node = dfs_tree_parent[current_node]
