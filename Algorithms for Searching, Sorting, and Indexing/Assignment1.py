...
def polynomial_multiply(a_coeff_list, b_coeff_list):
    # Calculate the length of the result polynomial
    result_length = len(a_coeff_list) + len(b_coeff_list) - 1

    # Pad both lists with zeros to make their lengths equal to result_length
    a_coeff_padded = a_coeff_list + [0] * (result_length - len(a_coeff_list))
    b_coeff_padded = b_coeff_list + [0] * (result_length - len(b_coeff_list))
    ...
    ifft_product = ifft(fft_product)

    # Convert the result from complex to real numbers
    result = [real(x) for x in ifft_product]

    # Truncate any tiny fractional parts that are effectively zero
    result = [round(x, 10) for x in result]  # Round to 10 decimal places
...
def polynomial_multiply_fft(a, b):
    # Length of the result polynomial
    result_length = len(a) + len(b) - 1

    # Pad both a and b with zeros to make their lengths equal to result_length
    a += [0] * (result_length - len(a))
    b += [0] * (result_length - len(b))

    # Compute FFT of both a and b
    fft_a = fft(a)
    fft_b = fft(b)
...
def check_sum_exists(a, b, c, n):
    ...
    # Convert sets a, b into polynomials
    for num in a:
        a_coeffs[num] = 1
    for num in b:
        b_coeffs[num] = 1
    ...
    c_coeffs = polynomial_multiply_fft(a_coeffs, b_coeffs)

    # Check if there is a number n3 in C such that n1+n2=n3
    for num in c:
        if num < len(c_coeffs) and c_coeffs[num] >= 1:
            return True  # Found a match where n1 + n2 = n3

    return False  # No such numbers found

# Example usage
a = {1, 3, 5}  # Subset A
b = {2, 4, 6}  # Subset B
c = {3, 7, 11} # Subset C
n = max(max(a), max(b), max(c))  # Maximum
...
# Read the CSV file and extract weekly prices
with open('natural_gas_futures_weekly_all.csv', 'r') as file:
    csv_handle = csv.DictReader(file)
    weekly_prices = [0.5 * (float(row['High']) + float(row['Low'])) for row in csv_handle]

# Plot the weekly prices
plt.plot(range(len(weekly_prices)), weekly_prices, '-b')
plt.xlabel('Week #')
plt.ylabel('Natural Gas Future Price')
plt.show()
...
# Compute frequencies
fft_frequencies = [(i if i <= N // 2 else i - N) / N for i in range(N)]
...
# Redefine the lists using the select_all_items function
...
# The resulting lists
print("Upto 1 year:", upto_1_year)
print("One year to 1 quarter:", one_year_to_1_quarter)
print("Less than 1 quarter:", less_than_1_quarter)
