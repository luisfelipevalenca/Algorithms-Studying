...
def findCrossoverIndex(x, y):
    ...
    # Start the recursive search within the entire array
...
def integerCubeRootHelper(n, left, right):
    ...
    while left <= right:
        ...
        if mid_cube == n:  # If we found the exact cube root, return it
            return mid
        elif mid_cube < n:  # If mid^3 < n, move the left bound to mid + 1
            left = mid + 1
        else:  # If mid^3 > n, move the right bound to mid - 1
            right = mid - 1

    # The while loop will exit when left > right, return the smaller value (right) as cube root
    return right
...
def twoWayMerge(lst1, lst2):
    ...
    # two ascending order sorted lists lst1 and lst2
    result = []
    i, j = 0, 0
    ...
    # Append any remaining elements from lst1 and lst2
