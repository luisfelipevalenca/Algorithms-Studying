...
def dpll_algorithm(formula, partial_truth_assign, j):
    if j > formula.n:
        result = formula.evaluate(partial_truth_assign)
        if result == 1:
            return True, partial_truth_assign
        else:
            return False, None

    extended_truth_assign = extend_truth_assignment(partial_truth_assign.copy(), j, True)
    res, truth_assignment = dpll_algorithm(formula, extended_truth_assign, j + 1)
    if res:
        return True, truth_assignment

    extended_truth_assign = extend_truth_assignment(partial_truth_assign.copy(), j, False)
    res, truth_assignment = dpll_algorithm(formula, extended_truth_assign, j + 1)
    if res:
        return True, truth_assignment
...
def translate_three_coloring(graph):
    ...
    for v in range(graph.n):
        s.add_clause([v * 3 + 1, v * 3 + 2, v * 3 + 3])
        for i in range(1, 4):
            for j in range(i + 1, 4):
                s.add_clause([-(v * 3 + i), -(v * 3 + j)])
    for edge in graph.get_list_of_edges():
        u, v = edge
        for i in range(1, 4):
            s.add_clause([-(u * 3 + i), -(v * 3 + i)])
    return s
...
def extract_graph_coloring_from_truth_assignment(graph, truth_assign):
    ...
    for v in range(graph.n):
        for i in range(1, 4):
            if truth_assign[v * 3 + i]:
                coloring[v] = i
    return coloring

# Solve the 3-coloring problem using a SAT solver
